...
# Ваши данные
APIkey = "AQVN2Gcnd6KX3Dy38mZEaeo-wZ2k839bGkGSWaGd"
folder_id = "e665012fba77d4cf4"
...
def search_yandex(query, api_key, folder_id):
    url = "https://yandex.ru/search/xml"  # Устаревший API, не рекомендуется использовать
    params = {
        "query": query,
        "apikey": api_key,
        "folderid": folder_id,
        "lr": 213,  # Регион (213 — Москва)
        "sortby": "rlv",  # Сортировка по релевантности
        "filter": "none",  # Без фильтров
        "groupby": "attr%3D%22%22.mode%3Dflat.groups-on-page%3D3.docs-in-group%3D1"  # 3 результата на странице
    }
...
with open('resultsYAN.csv', mode='w', newline='', encoding='utf-8') as file:
    writer = csv.writer(file)

    # Обрабатываем каждый запрос
    for query in queries:
        print(f"Обрабатывается запрос: {query}")
        result = search_yandex(query, APIkey, folder_id)
        if result:
            links = [item.get("link") for item in result.get("items", [])[:3]]  # Берём первые 3 ссылки
            writer.writerow(links)  # Записываем запрос и ссылки

print("Результаты сохранены в файл resultsYAN.csv")